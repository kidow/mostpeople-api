  ...
  passport.use(
    new KakaoStrategy(
      ...
      async (accessToken, refreshToken, profile, done) => {
        let payload = {}
        if (profile._json.kaccount_email_verified)
          payload.email = profile._json.kaccount_email
        else payload.providerId = profile.id
        try {
          const user = await User.findProfile(payload)
          if (user.status === 3)
            done(null, false, {
              message: '밴 당한 유저입니다.',
              code: 1003
            ...
            user.status = 1
              ...
              emailVerified: !!profile._json.kaccount_email_verified,
